...
class Admin(User):
    def __init__(self, name):
        super().__init__(name, is_admin=True) # we call User's __init__ method to overwrite is_admin to True

class Customer(User):
    def __init__(self, name):
        super().__init__(name)
        self.purchases = [] # set aditiona attribute
...
class Product: # does not inherit any class
    def __init__(self, name, price, vendor):
        self.name = name
        self.price = price
        self.vendor = vendor

class Purchase: # does not inherit any class
